const destroyHandler = async (user, arg) => {
	...
	try {
		...
		console.error(`${user.account.username} was not able to destroy building ${name} (level: ${level}), at the position ${position[0]}.${position[1]}. The arg: "${arg}" was used`);
...
const checkIfDestroyIsPossible = (user, arg) => {
	...
	const building = arg.match(/\d+.|,\d+/) ? user.empire.find(b => b.position[0] === parseInt(arg.split(".")[0]) && b.position[1] === parseInt(arg.split(".")[1])) : user.empire.find(b => {

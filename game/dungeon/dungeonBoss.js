const { generateDungeonBossRound, generateDungeonBossResult } = require("./embedGenerator");
...
const createDungeonBossRound = async (message, progress)=>{
    ...
    await message.channel.send(dungeonRound);
        ...
        collector.on("collect", async (result)=>{
            ...
            if (weaponAnswerFilter.some(alternative => alternative === result.content.toLowerCase())) {
...
const calculateDungeonResult = async (progress)=>{
        ...
        if (randomPlayer) {
            if (weaponInfo.type === "attack") {
                ...
                if (randomPlayer.hero.currentHealth - tempDamageGiven <= 0) {
                    progress.dungeon.helperIds = progress.dungeon.helperIds.filter(h=> h !== randomPlayer.account.userId);
                }
    ...
    if (progress.dungeon.helperIds.length === 0 || progress.dungeon.boss.stats.currentHealth <= 0 || progress.bossAttempts > 3) {
        progress.finish = true;
        progress.win = progress.dungeon.helperIds.length && progress.dungeon.boss.stats.currentHealth <= 0;
        if (progress.win) {

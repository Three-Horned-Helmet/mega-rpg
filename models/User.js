...
userSchema.methods.buyBuilding = function (building, buildingCost) {
  ...
  this.empire = this.empire.filter(
    (structure) =>
      !(
        structure.position[0] === building.position[0] &&
        structure.position[1] === building.position[1]
      )
  );
...
userSchema.methods.recruitUnits = function (unit, amount) {
  ...
  console.log(
    typeof this.army.units[unit.requirement.building][unit.name],
    typeof amount
  );
